...
# Plotting dendrogram
# Distance matrix
e <- dist(normdata,method = "euclidean")
fit <- hclust(e,method = "single")
# Display dendrogram
plot(fit)
plot(fit,hang = -1)
# Creating 3 clusters on the dendrogram
groups <- cutree(fit,k=3)
groups
# Adding rectangular border to the dendrogram
rect.hclust(fit,k=3,border = 'red')
# Creating a matrix for groups 
membership <- as.matrix(groups)
# Adding membership column in dataframe final
...
write_csv(final, "hclustcrime.csv")
