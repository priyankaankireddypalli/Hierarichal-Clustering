...
# Plotting Dendrogram
distauto <- dist(normautodata,method = "euclidean")
fitauto <- hclust(distauto,method = "complete")
plot(fitauto)
plot(fitauto,hang = -1)
groupsauto <- cutree(fitauto,k=3)
rect.hclust(fitauto,k=3,border = "red")
membership <- as.matrix(groupsauto)
final <- data.frame(membership,autodata1)
aggregate(autodata1[,c(3,10,13,14,15,16,17,22)],by=list(final$membership),FUN = mean)
...
aggregate(autodata1[, c(-1,-3,-7,-10,-13,-14,-15,-16,-17,-22)], by = list(final$membership), FUN = Mode)
library(readr)
write_csv(final,"hclustauto.csv")
