...
# Applying gower's dissimilarity matrix
library(cluster)
gower <- daisy(normdata,metric = 'gower')
fit <- hclust(gower, method = "complete")
# Display dendrogram
plot(fit) 
plot(fit, hang = -1)
...
final <- data.frame(membership, telecom)
aggregate(telecom[, c(6,9,13,25,26,27,28,29,30)], by = list(final$membership), FUN = mean)
...
aggregate(telecom[, c(-6,-9,-13,-25,-26,-27,-28,-29,-30)], by = list(final$membership), FUN = Mode)
library(readr)
write_csv(final, "Tele_customer_churn.csv")
