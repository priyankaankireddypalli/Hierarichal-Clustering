...
flagoutair <- (EastWestAirlines$Balance>c1)
table(flagoutair)
...
c2
# Flagging outliers
flagoutbm <- (EastWestAirlines$Balance>c2)
table(flagoutbm)
# Replacing the outliers
...
# Plotting Dendrogram
# Distance Matrix
dist <- dist(normdata,method = 'euclidean')
fit <- hclust(dist,method = 'complete')
# Displaying dendrogram
plot(fit)
plot(fit,hang = -1)
groups <- cutree(fit,k=3)
# Adding borders for the clusters
rect.hclust(fit,k=3,border = "red")
# Creating matrix for groups
membership <- as.matrix(groups)
# Adding membership column to the dataframe
final <- data.frame(membership,EastWestAirlines)
aggregate(EastWestAirlines[,2:12],by = list(final$membership),FUN = mean)
library(readr)
write_csv(final,"hclusteastwest.csv")
getwd()
